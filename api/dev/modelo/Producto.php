...
require_once dirname ( __FILE__ ) . "/../../../dev/recursos/FPDF/fpdf.php";
require_once dirname ( __FILE__ ) . "/../../../dev/recursos/FPDI/fpdi.php";
require_once dirname ( __FILE__ ) . "/../../../dev/recursos/PHPMailer/PHPMailerAutoload.php";
...
";
		$this->enviarEmail("Venta Realizada Dev", $mensajeAdmin, $emailAdmin, $pdf);
	...
	function guardarDatosOXXO($nombre, $ocupacion, $telefono, $celular, $email, $rfc, $beneficiario, $asociado, $idOrden, $vigencia)
	{
		...
		$productoDB->guardarOrden(Producto::MEDICA365, $asociado, "null", $idOrden, Producto::MEDICA365_PRECIO);
	...
	function construirPDFMedica($certificado, $nombre, $vigencia) {
		try {
			...
			$pdf = new FPDI();
			$pdf->AddPage();
			$pdf->setSourceFile(dirname ( __FILE__ ) . "/../../../dev/recursos/documentos/bienvenida-medica365.pdf");
			...
			$path = dirname ( __FILE__ ) . "/../../../dev/recursos/medica365-afiliados/".self::POLIZA."-".$certificado.".pdf";
